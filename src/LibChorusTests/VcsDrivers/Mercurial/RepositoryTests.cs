				...
				using (var file0 = testRoot.GetNewTempFile(true))
				{
					...
					using (var file1 = testRoot.GetNewTempFile(true))
					{
						...
						using (var file2 = testRoot.GetNewTempFile(true))
						{
							...
							// It did move.
							Assert.That(file0Rev.Number.LongHash, Is.EqualTo(testRev.Number.LongHash));
							Assert.That(file0Rev.Branch, Is.EqualTo(string.Empty)); // default branch returns string.Empty in mercurial.
							// SUT
							...
							Assert.That(file2Rev.Number.LongHash, Is.EqualTo(testRev.Number.LongHash));
							...
							testRev = repo.GetRevisionWorkingSetIsBasedOn();
							// It did move.
							Assert.That(file1Rev.Number.LongHash, Is.EqualTo(testRev.Number.LongHash));
							Assert.That(file1Rev.Branch, Is.EqualTo(string.Empty)); // default branch returns string.Empty in mercurial.
							// SUT
							Assert.That(repo.UpdateToLongHash(file2Rev.Number.LongHash), Is.EqualTo(HgRepository.UpdateResults.Success));
							testRev = repo.GetRevisionWorkingSetIsBasedOn();
							Assert.That(file2Rev.Number.LongHash, Is.EqualTo(testRev.Number.LongHash));
							...
							// It did move.
							Assert.That(file2Rev.Number.LongHash, Is.EqualTo(testRev.Number.LongHash));
							Assert.That(file2Rev.Branch, Is.EqualTo("newbranch"));
							// SUT
