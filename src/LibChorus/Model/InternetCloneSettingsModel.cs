...
namespace Chorus.Model
{
	public class InternetCloneSettingsModel : ServerSettingsModel
	{
		...
		public string TargetDestination
		{
			get
			{
				if(string.IsNullOrEmpty(LocalFolderName) || string.IsNullOrEmpty(LocalFolderName.TrimEnd()))
				{
					return "";
				}
				return Path.Combine(ParentDirectoryToPutCloneIn, LocalFolderName.TrimEnd());
			}
		}
