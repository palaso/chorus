	...
	public class GetCloneFromInternetModel : InternetCloneSettingsModel
	{
		...
		public override void DoClone()
		{
			...
			catch (Exception error)
			{
				...
				{
					_progress.WriteError(LocalizationManager.GetString("Messages.ServerRejectedLogon", "The server {0} did not accept the request of {1} to clone from {2}."), Host, Username, ProjectId);
					ErrorReport.NotifyUserOfProblem(LocalizationManager.GetString("Messages.RejectedLogonDetails", "The server ({0}) rejected the project name ({1}) or username ({2}) (sorry, it didn't tell us which one). Make sure that each of these is correct, and that '{2}' is a member of the '{1}' project, with permission to read data."),
